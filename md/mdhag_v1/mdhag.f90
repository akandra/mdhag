program mdhag
    ...
    call emt(slab, teilchen, Eref)
    teilchen%a=teilchen%f/mass_p
    teilchen%ao=0.0d0
    slab%a=slab%f/mass_p
    slab%ao=0.0d0
    ...
    do itraj = start_tr, ntrajs+start_tr-1
        ...
        ! This loop runs over the single steps inside one trajectory
        do q = 1, nsteps
            ! Beeman Algorithm

            ! Step 1: Calculate positions at t + dt. This needs to be called up seperately
            !         for the teilchen and the slab.
            call beeman_1(slab%r,slab%v,slab%a,slab%ao)
            call beeman_1(teilchen%r,teilchen%v,teilchen%a,teilchen%ao)


            ! Step 2 : Predict velocities from positions.
            call predict(slab%v,slab%vp,slab%a,slab%ao)
            call predict(teilchen%v,teilchen%vp,teilchen%a,teilchen%ao)

            ! Step 3: calculate accelerations form forces out of positions and predicted velocities.
